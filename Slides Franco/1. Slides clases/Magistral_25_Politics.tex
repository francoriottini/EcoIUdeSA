            ...
            \begin{center}
                \begin{minipage}[b]{0.45\textwidth}
                    \begin{center}
                        \begin{tikzpicture}[scale=0.4]
                            ...
                            \draw[thin](0.5,1.5)--(8.5,9) node [above] {\scriptsize $O_{FP} = O_{FP}^{''}$};
                            \draw[thin](2.5,9)--(10.5,1.2) node [right] {\scriptsize $D_{FP}$};
                            \draw[fill] (5.4,6.15) circle [radius =0.11] node [below] {\scriptsize A = \textcolor{green!50!black}{C}};
                            \draw[thick,dashed,gray] (0,6.2)--(5.4,6.2);
                            \node[left, blue] at(0,4.9) {\scriptsize $i_1$};
                            \draw[thin, blue](3,1.5)--(11,9) node [below right] {\scriptsize $O_{FP}^{'}$};
                ...
                \begin{minipage}[b]{0.45\textwidth}
                    \begin{center}
                        ...
                        \node[] at(5,-1.5) {\underline{\small Mercado de Trabajo}};
                        \draw[thin] (1,9).. controls (2,3) and (4, 2.5) .. (9, 1.5);
                        \draw[thin, green!50!black] (2,9).. controls (3,3.25) and (5, 3) .. (10, 2);
                        \node[right, green!50!black] at (10,2) {\tiny $T_d^{'}$};
                        \node[below] at (9.5,2) {\tiny $T_d$};
                        \draw[thin](3.5,2)--(3.5,9);
                        \draw[thick, dashed, gray] (3.5,2)--(3.5,0);
                        \node [] at (4.25,9) {\tiny $T_o$};
                        \node [below] at (3.5,0) {\tiny  $T^*$};
                        \draw[thin, dashed,gray] (0,3.35)--(3.5,3.35);
                        \node [left] at (0,3.25) {\tiny  $W^*$};
                        \draw[thin, dashed, green!50!black] (0,4.75)--(3.5,4.75);
                        \node [left, green!50!black] at (0,4.75) {\tiny  $W^{**}$};
                        \node [left] at (0,2) {\tiny  $\bar{W}$}  ;
                        \draw[thin] (0,2)--(3.5,2);
                        \draw[fill](3.5,3.35) circle [radius =0.1];
                        \draw[fill, green!50!black](3.5,4.75) circle [radius =0.1];
                        \end{tikzpicture}
                        \end{center}
                        \end{figure}
                        \end{center}
...
\end{center} 
    ...
    \begin{itemize}
        ...
        \only<2->{
            \item \textcolor{green!50!black}{A medida que suben los precios la demanda de dinero aumenta y también la tasa de interés. $\rightarrow$ En el mercado de credito, la oferta de FP se desplaza hacia la izquierda (alternativamente se puede pensar que los consumidores demandan más credito para financiar su consumo) produciendo el crowding out. Este \textit{crowding out} se da en conjunto con un aumento en los precios, por ende me desplaza a lo largo de la DA.} 
        }
...
\begin{frame}{Expansión fiscal financiada con deuda y agentes no ricardianos (keynesiano)}
    \begin{center}
        ...
        \node[left] at (0,6.05) {\footnotesize $P_1$};
        \node[left] at (0,10) {\footnotesize $P_2$};
        ...
        \draw[thin] (2.25,9).. controls (2.75,4) and (7, 3) .. (9, 2.75) node [right]{\footnotesize $M^{d^{'}} (P_1, Y_1)$};
        % \draw[thin, gray, dashed] (3,9).. controls (3.5,4) and (8, 3.5) .. (9,3.5) node [right]{\footnotesize $M^{d^{''}} (P_3, Y_0)$};
        \draw[thin, gray, dashed] (3,9).. controls (3.5,4) and (8, 3.5) .. (9,3.5) node [right]{\footnotesize $M^{d^{''}} (P_2, Y_0)$};
        \draw[thin](3.5, 0)--(3.5, 9) node [above]{\footnotesize $M^{o}$};
        \draw[thick, gray, dashed] (3.5,3.35)--(0,3.35);
        ...
        \draw[fill] (3.5,6.7) circle [radius =0.11] node[right] {\scriptsize C}; 
        \node[below] at (3.5,0) {\footnotesize $M_0$};
...
\node[box, below=of imp, xshift=1.5cm] (imp_trans) {Transitorio$^\mathbf{*}$};
...
\node[box, below=of deuda, xshift=2cm] (noric) {Agentes\\ No Ricardianos$^\mathbf{*}$};

% Subniveles ricardianos
\node[box, below=of ric, xshift=-1.5cm] (ric_perm) {Permanente};
\node[box, below=of ric, xshift=1.5cm] (ric_trans) {Transitorio$^\mathbf{*}$};
...
\node[anchor=north west, font=\scriptsize, yshift=-1.2cm, align=left, text width=15.5cm] at (current bounding box.south west) {($^\mathbf{*}$) El resultado sobre el producto dependerá del modelo: en el clásico, el efecto es neutral; en el keynesiano, puede ser expansivo en el corto plazo};
\end{tikzpicture}
}
\end{frame}
