...
\begin{frame}{Política Monetaria en el mundo clásico}
    ...
    \only<2->{
        \draw[thick, blue, dashed] (4,5)--(0,5);
        \node[left, blue] at (0,5) {$p_1$};
        ...
        \draw[fill, blue] (4,5) circle [radius =0.11] node[above right] {$C$};
    }
    \end{tikzpicture}
    \end{center}
    \end{minipage}
    ...
    \draw[fill] (6.5,2.05) circle [radius =0.11] node[right] {$B$};
    \node[left] at (0,3.35) {$i_0$};
    \only<2->{
        \draw[thick, gray, dashed] (6.5,3.35)--(3.5,3.35);
        \draw[semithick, blue] (2.25,9).. controls (2.75,4) and (7, 3) .. (9, 2.75) node [right]{\footnotesize $M^{d'}(P_1,\, Y_0)$}; 
        \draw[fill, blue] (6.5,3.35) circle [radius =0.11] node[above right] {$C$};
    }
    \end{tikzpicture}
    \end{center}
    \end{minipage}
    ...
    \begin{itemize}
        ...
        \item A ese nivel de precios hay un exceso de demanda en el mercado de bienes y una baja en la tasa de interés en el mercado de dinero (y en el de crédito por desplazamiento de la oferta de FP).
        \only<2->{
        \item \textcolor{blue}{Eso presiona hacia arriba los precios e incrementa la demanda de dinero (lo que reduce la oferta de FP en el mercado de crédito).}}
    \end{itemize}
\end{frame}
